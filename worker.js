...
export class Room {
  ...
  async fetch(request) {
    if((request.headers.get('Upgrade')||'').toLowerCase()!=='websocket') return new Response('Room online');
  ...
  message(id,m){const p=this.players.get(id);if(!p)return;
    ...
    if(m.type==='weaponSwitch'){const w=m.weapon==='bow'?'bow':'sword';p.weapon=w;this.broadcastPlayers();return;}
